"""The docutils components Sphinx domain.

Registers six object types (``transform``, ``reader``, ``parser``,
``writer``, ``node``, ``translator``) with matching cross-reference
roles, one grouped-by-objtype index, and the standard lifecycle hooks
Sphinx expects from a parallel-safe domain.

The component autodoc directives wire into this domain by generating
``.. docutils:<objtype>:: dotted.path.ClassName`` markup, so the parsed
``desc`` nodes natively carry ``domain="docutils"`` and a per-type
``objtype`` — the shared layout and badge pipelines key off both.

Examples
--------
>>> from sphinx_autodoc_docutils.domain import DocutilsDomain
>>> DocutilsDomain.name
'docutils'
>>> sorted(DocutilsDomain.object_types)
['node', 'parser', 'reader', 'transform', 'translator', 'writer']
>>> sorted(DocutilsDomain.roles) == sorted(DocutilsDomain.object_types)
True
>>> [cls.name for cls in DocutilsDomain.indices]
['componentindex']
"""
...
#: Object type name used for docutils transforms.
TRANSFORM = "transform"
#: Object type name used for docutils readers.
READER = "reader"
#: Object type name used for docutils parsers.
PARSER = "parser"
#: Object type name used for docutils writers.
WRITER = "writer"
#: Object type name used for custom docutils node classes.
NODE = "node"
#: Object type name used for docutils translator classes.
...
def split_component_path(path: str) -> tuple[str, str]:
    ...
    Examples
    --------
    >>> split_component_path("docutils.transforms.misc.Transitions")
    ('docutils.transforms.misc', 'Transitions')

    >>> split_component_path("Transitions")
    ('', 'Transitions')
    """
...
class DocutilsComponentDescription(ObjectDescription[str]):
    """Object description for one docutils component class.
    ...
    target are owned by :class:`DocutilsDomain`.
    """
    ...
    def add_target_and_index(
        ...
        node_id = make_id(
            self.env,
            self.state.document,
            f"docutils-{self.objtype}",
            name,
        )
        signode["ids"].append(node_id)
        self.state.document.note_explicit_target(signode)
        domain = t.cast("DocutilsDomain", self.env.domains[DocutilsDomain.name])
        domain.note_component(self.objtype, name, self.env.docname, node_id)


class DocutilsComponentIndex(Index):
    """Grouped-by-objtype index of every registered docutils component.

    The generated page lives at ``docutils-componentindex.html`` and can
    be linked via ``:ref:`docutils-componentindex```.

    Examples
    --------
    >>> DocutilsComponentIndex.name
    'componentindex'
    >>> str(DocutilsComponentIndex.localname)
    'Docutils components index'
    """

    name = "componentindex"
    localname = _("Docutils components index")
...
class DocutilsDomain(Domain):
    ...
    object_types = {  # noqa: RUF012 — matches upstream sphinx.domains.Domain shape
        TRANSFORM: ObjType(_("transform"), TRANSFORM),
        READER: ObjType(_("reader"), READER),
        PARSER: ObjType(_("parser"), PARSER),
        WRITER: ObjType(_("writer"), WRITER),
        NODE: ObjType(_("node"), NODE),
        TRANSLATOR: ObjType(_("translator"), TRANSLATOR),
    }

    directives = {  # noqa: RUF012 — matches upstream sphinx.domains.Domain shape
        TRANSFORM: DocutilsComponentDescription,
        READER: DocutilsComponentDescription,
        PARSER: DocutilsComponentDescription,
        WRITER: DocutilsComponentDescription,
        NODE: DocutilsComponentDescription,
        TRANSLATOR: DocutilsComponentDescription,
    }
    ...
    def _lookup(self, typ: str, target: str) -> tuple[str, str] | None:
        ...
        Authors commonly write the bare class name
        (``SanitizeTransform``); fall back to a suffix match when it
        identifies exactly one component.
        """
