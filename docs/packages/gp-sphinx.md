...
- Auto-computed SEO values when `docs_url` is set: `ogp_site_url`, `ogp_site_name`, `ogp_image` for {doc}`sphinx-gp-opengraph`, plus `site_url` and `sitemap_url_scheme` for {doc}`gp-sitemap`. See {ref}`from-docs_url` for the canonical mapping.
...
`sphinx_gp_opengraph` and `gp_sitemap` are members of
