...
| `docs_url` | `str \| None` | `None` | Canonical docs URL. When set, auto-derives `ogp_site_url`, `ogp_site_name`, `ogp_image` (for `gp_opengraph`) and `site_url`, `sitemap_url_scheme` (for `gp_sitemap`) — see {ref}`from-docs_url` |
